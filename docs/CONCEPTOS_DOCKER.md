...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8061
EXPOSE 8061
...
| `EXPOSE 8061` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            FRONTFLASK["SERVIDOR WEB (front)<br/>contenedor front-flask<br/>hostname: front-flask · escucha en 8062"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8061"]
    ...
    NAV -->|"localhost:8061"| APIFACTURAS
    NAV -->|"localhost:8062"| FRONTFLASK
    ...
    FRONTFLASK -->|"http://api-facturas:8061<br/>al controlador, por el NOMBRE"| APIFACTURAS
    SQLSERVERINIT -->|"espera el healthcheck,<br/>siembra y termina"| SQLSERVER
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:13351"| MARIADB
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15457"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11457"| SQLSERVER
```
...
```yaml
  postgres:
    ...
    ports:
      - "15457:5432"                 # "puerto en su PC : puerto interno"
...
1. **Dos redes de nombres**: hacia su PC, puertos publicados
   (`localhost:8061`, `localhost:15457`); entre contenedores, nombres de
...
docker network create proyecto_diseno_de_software6_default

# 2. postgres
docker run -d --name postgres --network proyecto_diseno_de_software6_default --restart unless-stopped `
  ...
  -p 15457:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver
docker run -d --name sqlserver --network proyecto_diseno_de_software6_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Diseno123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11457:1433 mcr.microsoft.com/mssql/server:2022-latest

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_diseno_de_software6_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Diseno123!" `
  -v "${PWD}/db:/scripts:ro" mcr.microsoft.com/mssql/server:2022-latest /scripts/init_sqlserver.sh

# 7. mariadb
docker run -d --name mariadb --network proyecto_diseno_de_software6_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_diseno_de_software6_default --restart unless-stopped `
  ...
  -v /app/bin `
  -v /app/obj `
  -p 8061:8061 api-facturas

# 10. Construir la imagen de front-flask y encenderla
docker build -t front-flask ./front_flask
docker run -d --name front-flask --network proyecto_diseno_de_software6_default --restart unless-stopped `
  -e "API_FACTURAS_URL=http://api-facturas:8061" `
  -e "CLAVE_SESION=Diseno123!Sesion" `
  -v "${PWD}/front_flask:/app" `
  -p 8062:8062 front-flask
...
```yaml
URL_API: http://api-facturas:8061
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8061` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8061` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8061:8061`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8061`, el front
...
```powershell
docker network inspect proyecto_diseno_de_software6_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_diseno_de_software6` produce
la red `proyecto_diseno_de_software6_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_diseno_de_software6-front-flask-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_diseno_de_software6_default has active endpoints (name:"proyecto_diseno_de_software6-front-flask-1" …)
```
