...
```
front_flask/
├── Dockerfile                  ← python:3.12-slim + flask + requests (puerto 8062)
...
**Crecen:** `docker-compose.yml` (★ servicio `front-flask` :8062,
...
Servicio `front-flask`: build `./front_flask`, puerto **8062** (estudiante
8162), `API_FACTURAS_URL=http://api-facturas:8061` (el hostname interno),
`depends_on: api-facturas`, `restart: unless-stopped`. El navegador usa
`localhost:8062`; el front usa el DNS interno — nunca localhost.
