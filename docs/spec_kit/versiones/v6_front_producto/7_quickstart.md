...
Servicios: postgres · sqlserver (+init) · mariadb · api-facturas (:8061) ·
**front-flask (:8062)**.
...
curl.exe -X POST http://localhost:8061/api/usuario -H "Content-Type: application/json" -d "{\"email\":\"demo@correo.com\",\"contrasena\":\"Demo123!\"}"

# 3b. EN EL NAVEGADOR — http://localhost:8062
...
curl.exe http://localhost:8061/api/producto            # los mismos 8 de la tabla
```
