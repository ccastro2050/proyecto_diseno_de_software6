...
flowchart LR
    ...
    A["api_facturas<br/>C# / ASP.NET Core — :8061"]
    B[("PostgreSQL<br/>bdfacturas COMPLETA, dada<br/>:15457")]
...
En una frase: *alguien le habla JSON a la API por el 8061, y la API le
...
1. **`docker compose up -d --build` — un solo comando —** deja corriendo
   ...
   `GET http://localhost:8061/` responde el JSON de diagnóstico. Guardar un
