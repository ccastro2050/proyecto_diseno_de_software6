...
└── api_facturas/
    ...
    ├── Dockerfile                    # sdk:10.0 + dotnet watch (puerto 8061)
    ├── Modelos/
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna privada del compose (una LAN virtual)"]
            API["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname interno: api-facturas<br/>.NET 10 + dotnet watch · escucha en 8061"]
    ...
    NAV -->|"ÚNICA puerta publicada al exterior:<br/>localhost:8061"| API
    API -->|"por la LAN interna, por NOMBRE:<br/>postgres:5432 (DNS de Docker)"| PG
    NAV -.->|"puerta opcional de diagnóstico:<br/>localhost:15457 (DBeaver/pgAdmin)"| PG
```
...
publican las puertas que el compose declara (`8061` para usar el
sistema; `15457` solo para inspeccionar la BD con una herramienta). Por
...
funcionando. En v1 eso son **dos servicios**: `postgres` (15457 al host,
se siembra solo) y `api-facturas` (8061, código montado +
