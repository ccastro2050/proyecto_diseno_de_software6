...
| `docker-compose.yml` | ★ servicios `sqlserver` (2022, :11457, healthcheck) + `sqlserver-init` + variables `Motor` y `ConnectionStrings__SqlServer` en la API |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop"]
        subgraph RED["red interna del compose (LAN virtual)"]
            API["SERVIDOR DE APLICACIONES<br/>api-facturas · escucha en 8061"]
...
- Puerto publicado **11457** (curso; la reconstrucción del estudiante
  usa 11557).
