...
flowchart LR
    CLI["Cliente HTTP"]
    subgraph API["api_facturas :8061 — v4"]
    ...
    PG[("PostgreSQL :15457<br/>bdfacturas — se siembra sola")]
    SS[("SQL Server :11457<br/>la MISMA bdfacturas<br/>sembrada por sqlserver-init")]
...
- Servicio `sqlserver` (SQL Server 2022, ~2 GB de RAM) en el compose,
  puerto publicado **11457**, con healthcheck real (sqlcmd) y
