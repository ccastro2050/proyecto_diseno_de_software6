...
> ## 🌐 ENTRAR AL FRONT → **http://localhost:8062**
...
> el puerto 8061 (pasa al reiniciar el PC: la API vieja revive sin su
...
> ```
> NAMES                                         STATUS                    PORTS
> proyecto_diseno_de_software6-api-facturas-1   Up 2 hours                0.0.0.0:8061->8061/tcp
> proyecto_diseno_de_software6-postgres-1       Up 2 hours (healthy)      0.0.0.0:15457->5432/tcp
> ```
>
...
```powershell
git clone https://github.com/ccastro2050/proyecto_diseno_de_software6.git
cd proyecto_diseno_de_software6
...
| Qué | Dónde |
|---|---|
| **API Facturas** — diagnóstico | http://localhost:8061/ |
| **Swagger** (documentación interactiva: ver y probar los endpoints) | http://localhost:8061/swagger |
| **FRONT** (v6) — la aplicación web con la marca | http://localhost:8062 |
| ¿Cómo ENTRO al front? | 1º cree su usuario (el front no registra: lo valida la API): `curl.exe -X POST http://localhost:8061/api/usuario -H "Content-Type: application/json" -d "{\"email\":\"demo@correo.com\",\"contrasena\":\"Demo123!\"}"` — 2º entre con `demo@correo.com` / `Demo123!` |
| Listar productos | http://localhost:8061/api/producto |
| PostgreSQL (para SQLTools/pgAdmin, opcional) | `localhost:15457` · `postgres`/`Diseno123!` |
| SQL Server (opcional — v4) | `localhost,11457` · `sa`/`Diseno123!` |
| MariaDB (opcional — v5) | `localhost:13351` · `root`/`Diseno123!` |
...
> ℹ️ Este proyecto usa los puertos 8061 y 15457: si alguno ya está ocupado
...
│
├── api_facturas/                # LA API DE LA v1 — C#/ASP.NET Core (puerto 8061)
...
│   ├── appsettings.json         # Cadena de conexión (default localhost:15457)
